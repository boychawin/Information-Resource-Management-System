...
class Mpdf implements \Psr\Log\LoggerAwareInterface
{
	...
	function RestrictUnicodeFonts($res)
	{
		...
		if (count($res)) { // booking full list of available fonts if passed blank array
						...
						elseif (count($this->available_unifonts) == 1) {
							...
							if (!$found) {
								...
								if (in_array($ws, $this->available_unifonts)) {
									$family = $fas[1]; // booking $style as is
	...
	function GetJspacing($nc, $ns, $w, $inclCursive, &$cOTLdata)
	{
		...
		if ($kashida_present) {
			...
			// If not just booking to adjust word-spacing
	 ...
	 * @param bool   $close If false bookings buffers etc. in current state, so that it can continue a block etc.
	 */
	...
	function printcolumnbuffer()
	{
		...
		} // Columns not ended but new page -> align columns (can booking the columns alone - just tidy up the height)
	...
	function SubstituteHiEntities($html)
	{
		...
		// bookings in particular &lt; to distinguish from tag marker
	 ...
	 * Only exception - bookings low ASCII entities e.g. &lt; &amp; etc.
	 * bookings in particular &lt; to distinguish from tag marker
	 */
